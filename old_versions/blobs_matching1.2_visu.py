...
# "exponential" part
sigma  = 5.
dist_proba = np.exp(-0.5 * (dist / sigma**2))
# normalisation
norm_aux = np.repeat(dist_proba.sum(1), nb_3D_blobs).reshape((nb_2D_blobs,
                                                              nb_3D_blobs))
norm_aux[norm_aux == 0.] = 1.
proba_exp = dist_proba / norm_aux
...
new_blobs2D_centers = np.array([], ndmin=1)
for blob2D in np.arange(nb_2D_blobs):
    for blob3D in np.arange(nb_3D_blobs):
    ...
    if significant_neighbors.size > 2:
        print "--> exploding blob %d"
        ...
        #whitened = cl.vq.whiten(blob2D_vertices)
        new_centers = cl.vq.kmeans(blob2D_vertices, significant_neighbors.size-1)[0]
        mayavi.points3d(new_centers[:,0], new_centers[:,1],
                        new_centers[:,2],
                        scale_factor=1, color=(0.5, 0.8, 0.3))
        new_blobs2D_centers = np.concatenate((new_blobs2D_centers,
                                              np.ravel(new_centers)), 1)
    else:
        new_blobs2D_centers = np.concatenate((new_blobs2D_centers,
                                              blobs2D_centers[blob2D]), 1)
        """
        i = 0
...
new_blobs2D_centers = new_blobs2D_centers.reshape((-1,3))
...
### Compute distances between each pair of (3D blobs)-(2D blobs centers)
new_nb_2D_blobs = new_blobs2D_centers.shape[0]
dist = np.zeros((new_nb_2D_blobs, nb_3D_blobs))
dist_arg = np.zeros((new_nb_2D_blobs, nb_3D_blobs), dtype='int')
for k in np.arange(nb_3D_blobs):
    blob3D_vertices = mroi.discrete_features['position'][k]
    blob3D_vertices_aux = np.tile(blob3D_vertices.reshape(-1,1,3),
                              (1,new_nb_2D_blobs,1))
    blobs2D_centers_aux = np.tile(new_blobs2D_centers,
                              (blob3D_vertices.shape[0],1,1))
    dist_all = ((blob3D_vertices_aux - blobs2D_centers_aux)**2).sum(2)
    dist_arg[:,k] = np.argmin(dist_all, 0)
    dist[:,k] = np.amin(dist_all, 0)
...
dist_proba = np.exp(-0.5 * (dist / sigma**2))
# normalisation
norm_aux = np.repeat(dist_proba.sum(1), nb_3D_blobs).reshape((new_nb_2D_blobs,
                                                              nb_3D_blobs))
norm_aux[norm_aux == 0.] = 1.
proba_exp = dist_proba / norm_aux
# "volume repartition" part
...
probaH1 = proba_exp * proba_rep
## "final" proba
proba = np.zeros((new_nb_2D_blobs, nb_3D_blobs+1))
proba[:,0:-1] = (probaH1 * gamma) / \
                ((1.-nb_3D_blobs*gamma)*probaH0 + \
                 gamma*np.tile(probaH1.sum(1), (nb_3D_blobs,1)).T)
proba[:,nb_3D_blobs] = (probaH0[:,0]*(1-nb_3D_blobs*gamma)) / \
...
ax.text((blobs3D_x-blobs2D_x)/2, 6,
        ...
        horizontalalignment='center')
ax.set_xlim(blobs2D_x-10, blobs3D_x+4)
ax.set_ylim(-blobs2D_spacing*np.amax([new_nb_2D_blobs-1, nb_3D_blobs-1]),
            2*blobs2D_radius)
ax.xaxis.set_visible(False)
ax.yaxis.set_visible(False)
for blob2D in np.arange(new_nb_2D_blobs):
    p = mpatches.Circle((blobs2D_x,-(blobs2D_spacing*blob2D)), blobs2D_radius)
    ax.text(blobs2D_x-5, -(blobs2D_spacing*blob2D)-1, '%d'%blob2D,
            horizontalalignment='center')
    ax.add_patch(p)
...
blobs3D_lines_colors = np.tile(['b','g','c','m','k','y'], 10)
# customize the output text results
res_file = open('./results/ver0/new_res.txt', 'w')
sys.stdout = res_file
print "### SUBJECT: %s" %SUBJECT
print "# Contrast: %s" %CONTRAST
print "# gamma: %g\n" %gamma
print "nb_2D_blobs = %d" %new_nb_2D_blobs
print "nb_3D_blobs = %d\n" %nb_3D_blobs
    ...
    for blob3D in np.arange(nb_3D_blobs):
        if proba[blob2D, blob3D] > 7.5e-1:
            ax.plot([blobs2D_x,blobs3D_x],
                    [-(blobs2D_spacing*blob2D),blobs3D_pos[blob3D]],
                    color=blobs3D_lines_colors[blob3D])
            point2D = new_blobs2D_centers[blob2D,:]
            ...
            mayavi.points3d(new_blobs2D_centers[blob2D,0],
                            new_blobs2D_centers[blob2D,1],
                            new_blobs2D_centers[blob2D,2],
