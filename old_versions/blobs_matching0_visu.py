...
dist = np.zeros((nb_2D_blobs, nb_3D_blobs+1))
dist_arg = np.zeros((nb_2D_blobs, nb_3D_blobs+1), dtype='int')
...
#####################################################
match_2D_blobs = np.argmin(dist[:,0:nb_3D_blobs], 1)
...
d0 = 11.
d0 = d0**2
last_row = d0 * np.ones(nb_2D_blobs)
dist[:,nb_3D_blobs] = last_row
dist_proba = np.exp(-0.5 * (dist / sigma**2))
norm_aux = np.repeat(dist_proba.sum(1), nb_3D_blobs+1).reshape((nb_2D_blobs,
                                                              nb_3D_blobs+1))
norm_aux[norm_aux == 0.] = 1.
proba = dist_proba / norm_aux
...
ax.text((blobs3D_x-blobs2D_x)/2, 6,
        "Subject %s, Contrast %s" %(SUBJECT, CONTRAST),
...
for blob2D in np.arange(nb_2D_blobs):
    ...
    print "Naive approach: %d" %(match_2D_blobs[blob2D]+1)
    print "\n"
