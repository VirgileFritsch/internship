...
if SUBJECT == "group":
    ...
    OUTPUT_DIR = "%s/results" %(m_path)
    lresults_output = "%s/left_%s_%s_results.tex" % (OUTPUT_DIR, GA_TYPE, CONTRAST)
    rresults_output = "%s/right_%s_%s_results.tex" %(OUTPUT_DIR, GA_TYPE, CONTRAST)
    OUTPUT_ENTIRE_DOMAIN_DIR = "%s/results_entire_domain" %(m_path)
    lresults_entire_domain_output = "%s/left_%s_%s_results_entire_domain.tex" % (OUTPUT_ENTIRE_DOMAIN_DIR, GA_TYPE, CONTRAST)
    rresults_entire_domain_output = \
        "%s/right_%s_%s_results_entire_domain.tex" % (OUTPUT_ENTIRE_DOMAIN_DIR, GA_TYPE, CONTRAST)
    OUTPUT_AUX_DIR = "%s/results_aux" %(m_path)
    lresults_aux_output = "%s/left_%s_%s_results_aux.tex" \
                          % (OUTPUT_AUX_DIR, GA_TYPE, CONTRAST)
    rresults_aux_output = "%s/right__%s_results_aux.tex" \
                          % (OUTPUT_AUX_DIR, GA_TYPE, CONTRAST)
    OUTPUT_LARGE_AUX_DIR = "%s/results_aux_large" %(m_path)
    lresults_aux_large_output = "%s/left_%s_%s_results_aux_large.tex" \
                          % (OUTPUT_LARGE_AUX_DIR, GA_TYPE, CONTRAST)
    rresults_aux_large_output = "%s/right_%s_%s_results_aux_large.tex" \
                          % (OUTPUT_LARGE_AUX_DIR, GA_TYPE, CONTRAST)
    OUTPUT_COORD_DIR = "%s/results" %(m_path)
    lresults_coord_output = "%s/left_%s_%s_results_coord.tex" \
                            % (OUTPUT_COORD_DIR, GA_TYPE, CONTRAST)
    rresults_coord_output = "%s/right_%s_%s_results_coord.tex" \
                            % (OUTPUT_COORD_DIR, GA_TYPE, CONTRAST)
...
if blobs2D_to_show[0] == -3.:
    ...
    for b in Blob2D.nodes.values():
        ...
        for i in b.vertices_id:
            if the_tex[i] == -1:
                the_tex[i] = -0.7
    # write results
    if not os.path.exists(OUTPUT_ENTIRE_DOMAIN_DIR):
        os.makedirs(OUTPUT_ENTIRE_DOMAIN_DIR)
    output_entire_domain_rtex = tio.Texture(rresults_entire_domain_output,
                                            data=rtex_entire)
    output_entire_domain_rtex.write()
    output_entire_domain_ltex = tio.Texture(lresults_entire_domain_output,
                                            data=ltex_entire)
    ...
    # write results
    if not os.path.exists(OUTPUT_LARGE_AUX_DIR):
        os.makedirs(OUTPUT_LARGE_AUX_DIR)
    output_aux_large_rtex = tio.Texture(rresults_aux_large_output, data=rtex_aux)
    output_aux_large_rtex.write()
    output_aux_large_ltex = tio.Texture(lresults_aux_large_output, data=ltex_aux)
