...
def phi(dik):
    ...
    #return (35./32.)*(1-((dik-75.)/75.)**2)**3   #triweight kernel

def plot_matching_results(nb_2Dblobs, nb_3Dblobs, proba, dist,
                          gamma, gamma_prime, sigma, file, blobs2D_list,
                          explode=False):
    global SUBJECT, CONTRAST, threshold_sure
    global threshold_maybe, threshold_explode, threshold_significant
    ...
    for blob2D in np.arange(nb_2Dblobs):
        ...
        significant_neighbors = np.where(proba[blob2D,:] > threshold_significant)[0]
        ...
        if (np.where(proba[blob2D,:] > threshold_explode)[0].size > 2 and explode):
...
threshold_explode = 5e-2
threshold_significant = 1e-2 #not conditioning results (< threshold_significant)
