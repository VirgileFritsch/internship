...
blobs2D = np.concatenate((blobs2D_rtex.data, blobs2D_ltex.data))
nb_2D_blobs = int(np.amax(blobs2D_rtex.data) + np.amax(blobs2D_ltex.data) + 2)
...
blobs2D_centers = np.zeros((nb_2D_blobs, nb_3D_blobs, 3))
for blob3D in np.arange(nb_3D_blobs):
    blob3D_vertices = mroi.discrete_features['position'][blob3D]
    # process right hemisphere 2D blobs
    for blob2D in np.arange(nb_right_blobs):
        blob2D_vertices = rvertices[np.where(blobs2D_rtex.data == blob2D)]
        # select some "accurate" vertices
        blob3D_vertices_aux = np.tile(blob3D_vertices.reshape(-1,1,3),
                                      (1,blob2D_vertices.shape[0],1))
        blob2D_vertices_aux = np.tile(blob2D_vertices,
                                      (blob3D_vertices.shape[0],1,1))
        dist_all = ((blob3D_vertices_aux - blob2D_vertices_aux)**2).sum(2)
        dist_tmp = np.amin(dist_all, 0)
        frontier = np.where(dist_tmp < np.median(dist_tmp))
        # compute a current blob3D-adapted 2D blob center
        blob2D_center = get_2D_blob_center(blob2D_vertices[frontier])
        #mayavi.points3d(blob2D_center[0], blob2D_center[1],
        #                blob2D_center[2],
        #                scale_factor=1, color=(0., 0.5, 0.5))
        final_dist = ((blob3D_vertices - blob2D_center)**2).sum(1)
        dist_arg[blob2D,blob3D] = np.argmin(final_dist, 0)
        dist[blob2D,blob3D] = np.amin(final_dist, 0)
        blobs2D_centers[blob2D,blob3D,:] = blob2D_center
    
    # process left hemisphere 2D blobs
    for blob2D in np.arange(nb_left_blobs):
        blob2D_vertices = lvertices[np.where(blobs2D_ltex.data == blob2D)]
        # select some "accurate" vertices
        blob3D_vertices_aux = np.tile(blob3D_vertices.reshape(-1,1,3),
                                      (1,blob2D_vertices.shape[0],1))
        blob2D_vertices_aux = np.tile(blob2D_vertices,
        ...
        #                scale_factor=1, color=(0., 0.5, 0.5))
        final_dist = ((blob3D_vertices - blob2D_center)**2).sum(1)
        dist_arg[blob2D+nb_right_blobs,blob3D] = np.argmin(final_dist, 0)
        dist[blob2D+nb_right_blobs,blob3D] = np.amin(final_dist, 0)
        blobs2D_centers[blob2D+nb_right_blobs,blob3D,:] = blob2D_center
...
for blob2D in np.arange(nb_2D_blobs):
    for blob3D in np.arange(nb_3D_blobs):
        if proba[blob2D, blob3D] > 7.5e-1:
            ...
            point2D = blobs2D_centers[blob2D,blob3D,:]
            ...
            mayavi.points3d(blobs2D_centers[blob2D,blob3D,0],
                            blobs2D_centers[blob2D,blob3D,1],
                            blobs2D_centers[blob2D,blob3D,2],
    ...
    print np.vstack((significant_neighbors_id,
                     100.*proba[blob2D,significant_neighbors])).T
    """
